...
description: 捕获用户的碎片化想法并形成可追踪的 IDEA 编号、点子卡、来源记录和初步研究简报；当用户说“记录这个点子”“碎片知识入库”“整理成点子卡”或“初步评估研究潜力”时使用。它只负责点子资产化和初筛；正式检索交给 `literature-search-map`，深度调研交给 `research-intelligence-report`，立项判断交给 `gap-to-topic`。
---
...
- 已有项目路径：写入用户指定项目工作区，优先使用 `04-决策记录/`、`06-报告材料/`、`10-批注/` 和必要的 `.research/` 索引。
...
输出结构统一使用 `templates/ideas/idea-card.md`、
`templates/ideas/research-brief.md`、`templates/ideas/source-log.md` 和
`templates/ideas/live-direction.md`。skill 负责填充模板，不另行维护第二套完整结构。
...
- `10-批注/`：人工阅读批注收件箱、处理记录和已处理归档，例如 `inbox.md`、`review-log.md`、`processed/YYYY-MM-processed.md`；活跃收件箱只保留待处理或需确认条目。
...
3. 分级处理并严格转交后续能力：
   ...
   - 需要外部检索时转交 `literature-search-map`；需要综合库内外证据形成深度报告时转交 `research-intelligence-report`。本 skill 不自行承担深度检索或正式研究缺口论证。
...
外部候选阅读清单用于保存“值得读、但还要进一步确认”的文献线索。它不是精读结论，也不是 Zotero 写入计划。优先使用 `templates/ideas/external-reading-candidates.md` / `.csv` 字段，每条至少包含：
...
- `suggested_collection`：建议导入 Zotero 后放入的项目用途文献集；用途明确时使用 `02-综述-review`、`04-方法-method` 等，尚不能判断时使用 `00-待分配-triage`
...
其中 `rs:read/todo` 的含义固定为“尚未生成读书卡”；`00-待分配-triage` 只表示尚未完成项目相关性与用途分配。生成读书卡后应更新阅读状态标签，但不因阅读状态改变项目用途 collection。
...
- 新增 idea 文件必须写入对应子目录；除 点子卡、研究简报、实时方向文档、来源记录 外，不在根目录新增材料；人工阅读批注写入 `10-批注/inbox.md`。
...
- 集中读书卡、阶段性报告、检索材料、候选池、矩阵和人工批注分别位于 `corpus/reading-cards/cards/`、`06-报告材料/`、`03-文献矩阵/01-检索路线与候选文献/`、`03-文献矩阵/01-检索路线与候选文献/`、`03-文献矩阵/06-矩阵缺口与技术路线/`、`10-批注/`，根目录未堆放这些材料。

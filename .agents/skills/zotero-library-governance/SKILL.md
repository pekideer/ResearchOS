...
- Zotero user ID，默认 `0`。
- 可选用户分类偏好或已有 taxonomy，仅作为当前 agent 判断时的约束，不作为关键词自动分类规则。
- 可选筛选要求：研究方向、研究方法、研究对象、期刊名、期刊级别、文献集、标签、年份范围。
- 可选输出目录，推荐课题目录下的 `03-文献矩阵/07-团队追踪/` 或 `03-文献矩阵/09-治理记录/`；ResearchOS 系统级人读报告进入 `docs/reports/library-governance/`，机器 CSV/JSON 写入低层机器留存区 `.researchos/outputs/machine/M-002-library-governance/`。
...
6. 使用 `prepare-corpus`、`build-agent-packet` 准备带来源和条目 key 的治理语料；工具不得调用语言模型 API，也不得以关键词命中直接生成科研标签。
7. 当前 ChatGPT/Codex agent 亲自判断研究方向、研究方法、研究对象、主题关系和治理建议，输出结构化语义结果。
8. 使用 `build-plan` 校验 agent 结果并生成 CSV/JSON 文献治理矩阵；确定性相似度只用于召回候选，不作为主题或重复结论。
9. 基于 agent 判断和校验后的矩阵生成治理报告，包括元数据缺失、无 文献集、无 标签、疑似重复、标签别名、文献集建议等。
...
- 语义分类必须给出可回溯证据；材料不足时留空、标注 `Unclassified` 或进入人工复核，不得用关键词脚本补齐。
- ResearchOS 代码不得调用通用语言模型 API或读取模型 API key；批量语义任务由当前 agent 读取语料包完成。
...
python tools\zotero\zotero_ai_governance.py build-plan --results-jsonl RESULTS.jsonl
```

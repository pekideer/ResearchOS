...
快速开始见 `QUICKSTART.md`。首次打开先做只读就绪检查；普通科研任务不要求预先安装 Python 或配置 Zotero。隐私和公开发布边界见 `PRIVACY.md`。
...
- ResearchOS 代码不另行调用通用语言模型 API，也不用关键词脚本代替科研语义判断；批量任务由工具准备语料包，再由当前 ChatGPT/Codex agent 完成判断。
...
Local API 只读读取条目和附件信息主要用于 `tools/zotero/zotero_library_index.py sync/watch` 维护父文档：
...
5. 同步盘、项目成果、阅读矩阵、读书卡索引、报告和交接文档中必须使用项目相对路径、`{PROJECT_ROOT}/...`、`{RESEARCHOS_ROOT}/...` 或 `root_key + project_relative_path`，不得写入 `C:\`、`D:\`、`E:\`、`file:///`、`OneDrive -` 等本机绝对路径。
6. 只有本机解释器、虚拟环境、临时目录、系统依赖等机器内部运行配置可以使用绝对路径，并应放在用户主目录配置、环境变量或被忽略的 `.local/` 中。
7. 如需显式配置本机路径，优先在用户主目录创建：
...
```json
{
  "researchos_root": "<LOCAL_RESEARCHOS_ROOT>",
  "projects_root": "<LOCAL_PROJECTS_ROOT>",
...
python tools\zotero\zotero_library_index.py sync --ocr
python tools\zotero\zotero_library_index.py summary --recover-stale
...
| 任务 | 默认入口 | 详细规则 |
|---|---|---|
| 从父文档读取条目或检索内容上下文 | `python tools\zotero\build_zotero_library_context_packet.py --profile content --item-key ITEMKEY --include-text` | `WORKFLOWS.md` 工作流 1、`RUNBOOKS/zotero-library-parent-documents.md` |
| Zotero 父文档维护或排障 | `tools/zotero/zotero_library_index.py`、`tools/zotero/zotero_fast_collection_sync.py` | `RUNBOOKS/zotero-library-parent-documents.md`、`TOOL_CONTRACTS/01-zotero-parent-documents.md` |
| 单篇读书卡与阅读总表 | `paper-deep-reading`、`tools\reading_cards\sync_reading_summary_table.py` | `WORKFLOWS.md` 工作流 1、1A，`RUNBOOKS/reading-card-governance.md` |
| PRISMA 状态与综述矩阵 | `tools\reading_cards\build_prisma_status_outputs.py`、`literature-matrix` | `WORKFLOWS.md` 工作流 2、2A |
...
| 共享 corpus 发布与项目写入接力 | `tools/runtime/publish_corpus.py`、`project_write_guard.py` | `WORKFLOWS.md` 工作流 0D、`TOOL_CONTRACTS/10-corpus-publication-and-project-write.md` |
| 本机 OCR 环境和 watcher | `tools\runtime\build_local_python_env.ps1`、`tools\zotero\start_zotero_library_watcher.ps1` | `TOOL_CONTRACTS/01-zotero-parent-documents.md`、`TOOL_CONTRACTS/07-runtime-ocr-local-env.md` |

直接读取 Zotero Local API、定位 PDF 或抽取 PDF 文本不是普通阅读和治理任务的默认入口；只有父文档缺失、过期、路径失效或排障时才使用，且不得读取或修改 `zotero.sqlite`。

Zotero 父文档和规范化全文使用同步盘 `corpus\` 入口；本地 `.researchos\` 是被 Git 忽略的运行区，不能保存唯一项目状态或唯一正式审计副本。跨端存储和角色边界见 `docs\governance\cross-device-storage-and-role-architecture.md`。
...
- `tools/zotero/zotero_local_api_cli.py`：Zotero Local API 只读排障和 PDF 文本抽取合并入口。
- `tools/zotero/zotero_ai_governance.py`：Zotero 文献库治理语料包和只读计划主入口；内容标签与库结构使用分离的任务、证据和 schema，语义由当前 ChatGPT/Codex agent 判断。
- `tools/project/create_project_workspace.py`：根据用户指定课题目录或课题名创建编号化输出目录。
...
- `.researchos/`：本机运行、缓存、调试和待晋升审计暂存区；不跨端、不进入 Git。
- 项目 `.research/`：项目 manifest、状态、交接、决策、审批和精简审计；不得存放可重建过程材料。

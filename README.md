...
ResearchOS 是 Codex 作为科研助理时使用的长期复用运行框架。它用于固定可复用的科研场景、触发规则、提示词、质量标准、上下文恢复和语料准备流程，让 Codex 直接承担文献阅读、综述、选题、论文写作、方法审查、结果叙事、证据审计和审稿回复等助理工作。

ResearchOS 不是“建构科研智能体”的开发项目，也不是具体科研项目仓库。代码只用于补足大模型对话本身无法完成的本地文件获取、PDF 文本抽取、Zotero 读写、批量语料准备、结构化数据转换和外部系统桥接。

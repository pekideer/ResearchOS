...
`corpus_snapshot.py` 只读计算共享 corpus 的确定性内容快照。`project_handoff.py` 使用该快照、Agent Core commit 和终端身份，对项目 `.research/handoff.yml` 执行 bootstrap、release、claim 和写入权检查；改变项目状态必须显式使用 `--apply`。

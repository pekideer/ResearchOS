...
{reading_card_output.rstrip()}
  ...
  run_state: ".research/run_state.json"
  run_log: ".research/run-log.jsonl"
...
def run_state_text(root: Path) -> str:
    return json.dumps(
        {
            "schema_version": 1,
...
def scaffold_files(
    ...
    files = {
        ...
        root / ".research" / "run_state.json": run_state_text(root),
        root / ".research" / "run-log.jsonl": "",
...
def audit_workspace(root: Path, registry_reading_cards: str | None) -> int:
    ...
    run_state = root / ".research" / "run_state.json"
    run_log = root / ".research" / "run-log.jsonl"
    ...
    if not run_state.exists():
        issues.append("缺少 .research/run_state.json，无法恢复当前任务快照。")
    else:
        try:
            state_data = json.loads(run_state.read_text(encoding="utf-8-sig"))
            if not isinstance(state_data, dict) or state_data.get("schema_version") != 1:
                issues.append("run_state.json 缺少受支持的 schema_version: 1。")
        except (json.JSONDecodeError, OSError):
            issues.append("run_state.json 不是有效 JSON。")

    if not run_log.exists():
        issues.append("缺少 .research/run-log.jsonl，无法追加最小运行历史。")
    ...
    print(f"run_state: {run_state if run_state.exists() else '(missing)'}")
    print(f"run_log: {run_log if run_log.exists() else '(missing)'}")

...
6. 只有父文档和 缓存均缺失时，才允许通过 `tools/zotero/zotero_library_index.py` 或 Zotero Local API 只读定位 PDF 并抽取文本；抽取后必须写回父文档或 `.research/fulltext_cache/` 供后续复用。
...
```powershell
python tools\project\extract_project_materials.py --project-root "课题目录" --output-dir "课题目录\.research\material_text"
```

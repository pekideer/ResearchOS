...
- 需要提醒哪些文献尚无读书卡、重要、缺少计划用途或缺少排除原因。
...
- `templates/literature/paper-reading-card.md`
...
- “待读”由是否存在读书卡判断；项目相关性与用途尚未完成分配时使用 `00-待分配-triage`，两者不得互相替代。
...
- `rs:read/todo`
- `rs:read/initial-card`（`skimmed`、`done` 统一映射）
- `rs:read/deep-read`（`deep` 映射）
- `rs:priority/core`
- `rs:priority/high`
- `rs:priority/normal`
- `rs:priority/low`

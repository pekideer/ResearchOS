...
- Zotero governance 主入口：`tools/zotero/zotero_ai_governance.py`；只生成 agent 语料包并校验 agent 结果，不调用语言模型 API。
...
1. 按 `docs/governance/cross-device-storage-and-role-architecture.md` 建立本地运行区契约、角色检查和项目交接协议。
2. 先对一个项目执行不删除旧文件的换端金丝雀，再迁移现有 `.research/` 中的缓存、预览和备份。
3. 对现有 `.researchos/outputs/` 分类：可再生过程材料保留在本地，长期需要的最小审计晋升到项目或共享语料层。
